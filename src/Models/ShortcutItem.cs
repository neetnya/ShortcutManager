...
{
    /// <summary>
    /// 存储路径。可能是相对路径（目标在程序目录内时，便于整体迁移），
    /// 做磁盘操作前请用 <see cref="FullPath"/>。
    /// </summary>
    public string Path { get; set; } = string.Empty;

    /// <summary>解析后的完整路径（相对路径按程序目录展开）。</summary>
    [JsonIgnore]
    public string FullPath => PathResolver.Resolve(Path);
    ...
    [JsonIgnore]
    public bool Exists => !string.IsNullOrWhiteSpace(Path) && (File.Exists(FullPath) || Directory.Exists(FullPath));
    ...
    /// <summary>用于比较的规范化路径键（按解析后的完整路径，忽略大小写与结尾分隔符）。</summary>
    [JsonIgnore]
    public string PathKey => NormalizePath(FullPath);
